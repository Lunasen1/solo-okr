...
- **AI 教练**（可选）：AI 拆目标、周复盘洞察、目标诊断——只给建议、不替你决定
...
## 🤖 AI 教练（可选，需自备 API Key）

内置三个 AI 助手，**只给建议、不替你决定**——目标永远是你自己的。

- **AI 拆目标**：写一句模糊目标，AI 拆成 2~4 条可量化的 KR 建议，逐条采纳
- **AI 周复盘洞察**：复盘时一键生成「本周推进了什么 / 哪条 KR 该砍 / 下周聚焦点」
- **AI 目标诊断**：月初一键检查目标是否可量化、信心指数是否矛盾

**配置**：「设置 → AI 教练」填你自己的 API Key，支持 DeepSeek / Kimi / 智谱 / 通义等 OpenAI 兼容模型。详见[申请教程](docs/申请API-Key教程.md)。

**数据隐私**：API Key 只存本机浏览器；只有点「AI 分析」时，那部分数据才发送给模型服务商。不点 AI，数据完全不离开本机。

**离线可用**：不配 Key 也能正常使用，基础 OKR 功能完全离线。
